<?php require_once('../Connections/conexion.php'); 
  ...
  mysql_select_db($database_conexion, $conexion);
		$query_SacarEnviados = sprintf("SELECT * FROM z_notifica WHERE autor=%s ORDER BY id DESC",$_SESSION['MM_Id'],"int");
...
<div id="principal">
  ...
  <div id="leftt">
    <div id="section_info">Mis Notificaciones</div>
 
<?php  if ($totalRows_SacarEnviados!=0) {?>
 <?php do { ?>
    <div id="section_l"><a href="<?php echo $urlWeb ?>mis_post.php?date=<?php echo $row_SacarEnviados['post'] ?>"><?php echo saber_titulo($row_SacarEnviados['post']) ?></a> (Comentario de <?php echo nombre($row_SacarEnviados['comenta']) ?>)</div>
